...
class DeepseekV2MoE(nn.Module):
    ...
    def forward_normal(
        ...
        if shared_output is not None and os.getenv('USE_SIMPLE_FARSKIP', "0") == "1":
        ...
        if os.getenv('USE_SIMPLE_FARSKIP', "0") == "1":
            routed_experts = tensor_model_parallel_all_reduce(final_hidden_states)
            return shared_output, routed_experts
...
class DeepseekV2AttentionMLA(nn.Module):
    ...
    def __init__(
        ...
        if self.gated_attention:
            if self.layer_id == 0:
                print('[Layer 0], Creating a gated attention layer')
    ...
    def forward_normal_prepare(
        ...
        return q, k, v, forward_batch, gate # TODO added here

    def forward_normal_core(self, q, k, v, forward_batch, gate=None):
    ...
    def forward_absorb_core(
        ...
        if self.gated_attention: # TODO ADDED
            assert gate is not None
...
class DeepseekV2SimpleFarSkipDecoderLayer(DeepseekV2DecoderLayer):
    def __init__(
            self,
            config: PretrainedConfig,
            layer_id: int,
        ...
        self.simple_farskip = os.getenv("USE_SIMPLE_FARSKIP", "0") == "1" # TODO added
        self.simple_farskip = not os.getenv("SIMPLE_FARSKIP_NULL_DEBUG", "0") == "1" # to test path there
        if self.simple_farskip:
            self.self_attn.reduce_results = True  # reduce in layer not communcaotr
    ...
    def forward(
        ...
        if self.simple_farskip:
            ...
            attn_input, _ = self.layer_communicator.prepare_attn(
                hidden_states_no_routed,
                None,
                forward_batch,
                quant_format,
            )
        ...
        if self.simple_farskip:
            ...
            mlp_input, residual = self.layer_communicator.prepare_mlp(
        ...
        if isinstance(self.mlp, DeepseekV2MLP):
            ...
            shared_output, routed_experts = self.mlp(
                mlp_input,
                forward_batch,
                should_allreduce_fusion,
                use_reduce_scatter,
            ...
            if self.simple_farskip:
                ...
                hidden_states = residual + routed_experts
            else:
                hidden_states = shared_output + routed_experts
...
class DeepseekV2Model(nn.Module):
    ...
    def __init__(
        ...
        if os.getenv("USE_SIMPLE_FARSKIP", "0") == "1":
            decoder_layer_cls = DeepseekV2SimpleFarSkipDecoderLayer
        else:
            decoder_layer_cls = DeepseekV2DecoderLayer

        self.layers, self.start_layer, self.end_layer = make_layers(
            config.num_hidden_layers,
            lambda idx, prefix: decoder_layer_cls(
                config=config,
                layer_id=idx,
                quant_config=quant_config,
                prefix=prefix,
    ...
    def forward(
        ...
        if os.getenv("USE_SIMPLE_FARSKIP", "0") == "1" and not os.getenv("SIMPLE_FARSKIP_NULL_DEBUG", "0") == "1":  # TODO added
            residual = None
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for name, loaded_weight in weights:
                ...
                for param_name, weight_name, shard_id in stacked_params_mapping:
                    ...
                    # Skip attention layers - gate_proj in attention is not merged with up_proj
                    if "self_attn" in name: # TODO added
                        continue
                    # We have mlp.experts[0].gate_up_proj in the checkpoint.
