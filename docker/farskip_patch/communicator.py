...
class LayerCommunicator:
    ...
    def prepare_mlp_no_all_reduce(
        ...
        """Prepare MLP when attention already did all-reduce (reduce_results=True).

        Only safe when mlp_mode == FULL (dense layers without enable_moe_dense_fully_dp).
        Raises error if called with incompatible mode.
        """
        if cache is not None:
            self._context.cache = cache
